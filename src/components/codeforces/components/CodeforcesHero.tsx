...
function InfoChip({
  ...
  return (
    ...
    >
      <span
        className="text-meta"
        style={{ color: "var(--text-muted)", fontSize: "0.65rem", letterSpacing: "0.06em", textTransform: "uppercase" }}
      >
        {label}
      </span>
      <span
...
export function CodeforcesHero({ userInfo, stats }: Props) {
  ...
  const regDate = userInfo.registrationTimeSeconds
    ? new Date(userInfo.registrationTimeSeconds * 1000).toLocaleDateString("en-US", {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
  ...
  return (
    <>
      ...
      <div
        ref={cardRef}
        className="cf-hero-card relative overflow-hidden rounded-2xl p-6 md:p-8"
        style={{
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8">
          ...
          <div className="relative shrink-0">
            ...
            <div
              ...
              style={{ border: `3px solid ${rankColor}`, boxShadow: `0 0 24px ${rankColor}30` }}
            >
              {userInfo.avatar ? (
                <Image
                  src={userInfo.avatar}
            ...
            {stats.isCurrentlyActive && (
              <div
                className="absolute -bottom-1 -right-1 flex items-center gap-1 px-2 py-0.5 rounded-full text-meta"
                style={{
              ...
              >
                <span
                  className="cf-badge-dot w-1.5 h-1.5 rounded-full"
                  style={{
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-col md:flex-row items-center md:items-start gap-2 md:gap-4 mb-2">
              ...
              <span
                className="px-3 py-1 rounded-full text-meta font-semibold capitalize"
                style={{
            ...
            <div className="flex flex-col md:flex-row items-center md:items-baseline gap-1 md:gap-3 mb-4">
              <span
                ...
                style={{ color: rankColor, fontSize: "clamp(2.5rem, 6vw, 4rem)", lineHeight: 1 }}
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-2 mb-4">
              <InfoChip label="Max Rating" value={userInfo.maxRating} color={maxRankColor} />
              <InfoChip label="Max Rank" value={userInfo.maxRank} color={maxRankColor} />
              <InfoChip label="Contests" value={stats.contests.length} />
              <InfoChip label="Contribution" value={`${userInfo.contribution > 0 ? "+" : ""}${userInfo.contribution}`} />
              <InfoChip label="Friend of" value={`${userInfo.friendOfCount}`} />
            </div>

            {/* Registration & org */}
            <div className="flex flex-wrap justify-center md:justify-start gap-4 text-sm" style={{ color: "var(--text-muted)" }}>
