...
export default function ProjectCard({
  ...
  useEffect(() => {
    if (!repoName) return;
    fetch(`https://api.github.com/repos/hasnat0006/${repoName}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  return (
    ...
    >
      ...
      {duration && (
        <div
          className="inline-flex items-center gap-2 px-2.5 py-1 rounded-md mb-3 text-xs font-mono"
          style={{
      ...
      {/* Title */}
      <h3
        className="text-lg font-bold font-mono mb-2 group-hover:opacity-80 transition-opacity"
      ...
      >
        {githubUrl && (
          <a
            ...
            className="flex items-center gap-2 text-xs font-mono transition-colors px-3 py-1.5 rounded-md border"
            style={{
              color: "var(--text-accent)",
              borderColor: "var(--border-accent)",
            }}
