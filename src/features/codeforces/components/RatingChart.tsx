...
import type { CFRatingEntry } from "@/types/codeforces";
...
export function RatingChart({ ratingHistory }: Props) {
    ...
    return (
      ...
      >
        <p style={{ color: "var(--text-muted)" }}>
          Not enough contest data for chart
        </p>
      </div>
    );
  }
  ...
  const linePath = ratingHistory
    .map(
      (e, i) =>
        `${i === 0 ? "M" : "L"} ${xScale(i).toFixed(1)} ${yScale(e.newRating).toFixed(1)}`,
    )
    .join(" ");
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const idx = Math.max(
      0,
      Math.min(n - 1, Math.round((relX / chartW) * (n - 1))),
    );
  ...
  const rankColor = getRankColor(
    ratingHistory[ratingHistory.length - 1].newRating >= 2600
      ? "grandmaster"
      : ratingHistory[ratingHistory.length - 1].newRating >= 2300
        ? "master"
        : ratingHistory[ratingHistory.length - 1].newRating >= 1900
          ? "expert"
          : ratingHistory[ratingHistory.length - 1].newRating >= 1600
            ? "specialist"
            : ratingHistory[ratingHistory.length - 1].newRating >= 1200
              ? "pupil"
              : "newbie",
  );
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          ...
          <div
            className="flex items-center gap-4 text-meta"
          ...
          >
            ...
            {[
              ...
              <circle
                ...
                style={{
                  animation: visible
                    ? "cf-dot-pop 0.4s ease-out 1.8s both"
                    : "none",
                }}
              >
                <title>
                  {label}: {ratingHistory[idx].newRating}
                </title>
              </circle>
            ))}
          ...
          {tooltip && (
            <div
              ...
              style={{
                ...
                left:
                  tooltip.x > W * 0.7 ? "auto" : `${(tooltip.x / W) * 100}%`,
            ...
            >
              ...
              >
                {new Date(
                  tooltip.entry.ratingUpdateTimeSeconds * 1000,
                ).toLocaleDateString("en-US", { dateStyle: "medium" })}
              </p>
              <div
                className="mt-1.5 space-y-0.5 text-meta"
                style={{ fontSize: "0.7rem" }}
              >
                <div className="flex justify-between gap-4">
                  <span style={{ color: "var(--text-muted)" }}>Old</span>
                  <span style={{ color: "var(--text-primary)" }}>
                    {tooltip.entry.oldRating}
                  </span>
                </div>
                <div className="flex justify-between gap-4">
                  <span style={{ color: "var(--text-muted)" }}>New</span>
                  <span style={{ color: "var(--text-primary)" }}>
                    {tooltip.entry.newRating}
                  </span>
                </div>
                <div className="flex justify-between gap-4">
